...
/**
 * @name ier.model.ActivateHoldSweepProcessingAction
 * @class Check the hold sweep state
 * @augments ecm.model.Action
 */
return dojo_declare("ier.model.actions.ActivateHoldSweepProcessingAction", [ecm_model_action], {

	canPerformAction: function(repository, itemList, listType, teamspace, resultSet) {
		var canPerform = this.inherited(arguments);
		if(canPerform){
			return itemList[0].attributes["SweepState"] == 3 ? true: false;
		}else{
			return canPerform;
		}
	}
